...
import (
	...
	// "github.com/shappy0/sassc/internal/utils"
)
...
func (t *Table) SetTableTitle(count int, a, b string) {
	if a != "" && b == "" {
		t.SetTitle(fmt.Sprintf(" [::b][%s]%s(%s)[%d] ", "#cccccc", strings.ToUpper(t.Title), a, count))
	} else if a != "" && b != "" {
		t.SetTitle(fmt.Sprintf(" [::b][%s]%s(%s/%s)[%d] ", "#cccccc", strings.ToUpper(t.Title), a, b, count))
	} else {
		t.SetTitle(fmt.Sprintf(" [::b][%s]%s [%d] ", "#cccccc", strings.ToUpper(t.Title), count))
	}
}
